        ...
        scene.executeWhenReady(() => {
            ...
            async function createGUI() {
                ...
                engine.runRenderLoop(() => {
                    ...
                    nFrames += 1;
                    let fpsRate = scene.getEngine().getFps();
                    if (nFrames > 0 && nFrames <= 1000) {
                        sumFrames += fpsRate;
                        if (fpsRate < 30) sumDropFrames++;
                        textblockQuestion.text = `FPS médio: ${(sumFrames/nFrames).toFixed(1).replace('.', ',')}  FPS drop: ${(sumDropFrames).toFixed(0).replace('.', ',')}/${nFrames}`;
                    }
